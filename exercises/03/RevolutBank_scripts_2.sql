...
VALUES('4938 3108 1742 9022  ', '111', '2031-12-31'),
    ('4938 3108 1742 9023  ', '222', '2020-12-31'),
    -- expired card
    ('4938 3108 1742 9024  ', '333', '2021-12-31'); -- expired card
GO

CREATE TRIGGER [Cards].[TR_ForDeleteCard]
ON [Cards].[Cards]
INSTEAD OF DELETE
AS
BEGIN
...
EXEC sys.sp_addextendedproperty
    @name=N'TableDescription',
    @value=N'Table is used for storing account information.',
    @level0type=N'SCHEMA',
    @level0name=N'Accounts',
    @level1type=N'TABLE',
...
VALUES('IT72W0300203280817392947896', 12000, 1, 1, 1, 1, 1),
    ('IT72W0300203280817392947897', 15000, 1, 2, 2, 2, 3);
GO

CREATE TRIGGER [Accounts].[TR_AfterInsertAccount]
...
(
    ...
    CONSTRAINT FK_Transactions_Currencies_CurrencyId FOREIGN KEY(CurrencyId) REFERENCES Currencies.Currencies(CurrencyId),
);

EXEC sys.sp_addextendedproperty
    @name=N'TableDescription',
    @value=N'Table is used for storing transaction information.',
    @level0type=N'SCHEMA',
    @level0name=N'Accounts',
    @level1type=N'TABLE',
...
VALUES(1, 'HN50IKQT54288734823397434219', 100, 1, '1232133'),
    (2, 'HN50IKQT54288734823397434220', 200, 2, '2023-01-01'),
    (1, 'HN50IKQT54288734823397434221', 300, 1, '2023-02-01');
GO
------------ Accounts.Transactions ------------

------------ SP ------------
CREATE PROCEDURE [Accounts].[UPS_GetTransactionsToBGN](@iAccountId INT)
AS
BEGIN
    SELECT
